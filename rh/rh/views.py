                ...
                if candidato:
                    if "message" in data:
                        ...
                        if has_document or has_image:
                            ...
                            if doc_pendente:
                                tipo_mapeado = doc_pendente.tipo
                                    ...
                                    try:
                                        ...
                                        if has_image:
                                            ...
                                            if is_valid:
                                                ...
                                                tipo_foto_rosto, _ = TipoDocumento.objects.get_or_create(
                                                    nome="FOTO_ROSTO",
                                                    defaults={'ativo': True}
                                                )
                                                ...
                                                if doc_pendente.tipo.nome.lower() == 'foto_rosto':
                                        ...
                                        if "fileName" in media_info and media_info["fileName"]:
                                            ...
                                            file_name = f"{doc_pendente.tipo.nome.lower()}_{candidato.id}.{extension}"
                                        ...
                                        try:
                                            ...
                                            tipo_mapeado_nome = tipo_documento  # Inicialmente, assume o tipo retornado pela IA
                                            
                                            # Dicionário de mapeamento entre resposta da IA e valores do modelo

                                            # Mapeamento DIRETO entre resposta da IA e nomes dos tipos no banco
                                            mapeamento_tipos = {
                                                # 📸 Foto
                                                'foto_3x4': 'foto_3x4',
                                                'foto': 'foto_3x4',
                                            ...
                                            for chave, valor in mapeamento_tipos.items():
                                                if tipo_documento and (chave in tipo_documento.upper() or tipo_documento.upper() == valor):
                                                    tipo_mapeado_nome = valor
                                                    break

                                            # Se não encontrou no mapeamento, define como 'OUTROS'
                                            if tipo_mapeado_nome not in mapeamento_tipos.values():
                                                tipo_mapeado_nome = 'OUTROS'

                                            # Buscar ou criar o tipo de documento
                                            ...
                                            # Adicionar observações sobre o tipo identificado
                                            if tipo_mapeado_nome == 'OUTROS' and '|' in tipo_documento:
                                                # Extrair a descrição detalhada após o caractere '|'
                                                descricao_detalhada = tipo_documento.split('|', 1)[1].strip()
                                                doc_pendente.observacoes += f"\n{descricao_detalhada}"
                                            else:
                                                doc_pendente.observacoes += f"\nTipo identificado pela IA: {tipo_documento}"
                                                doc_pendente.observacoes += f"\nTipo mapeado: {tipo_mapeado.nome}"
                                ...
                                if not doc_pendente:
                                    ...
                                    registrar_evento(
                                        ...
                                        observacoes=f"Documento {doc_pendente.tipo.nome} criado automaticamente"
                                    )
                                ...
                                if doc_pendente.tipo.nome.upper() == 'OUTROS':
                                    doc_pendente.status = 'invalido'
                                    
                                    # Verifica se há uma descrição detalhada do documento não reconhecido
                                    if '|' in tipo_documento:
                                        # Extrair a descrição detalhada após o caractere '|'
                                        descricao_detalhada = tipo_documento.split('|', 1)[1].strip()
                                        doc_pendente.observacoes = descricao_detalhada
                                    else:
                                        doc_pendente.observacoes = "Tipo não reconhecido através da inteligência artificial."
...
from .models import ConfiguracaoCobranca, ControleCobrancaCandidato, HistoricoCobranca, Candidato, Documento, TipoDocumento, AvaliacaoPeriodoExperiencia
from django.contrib.auth.decorators import login_required
...
@login_required
def get_pesquisa_details(request, token):
    try:
        # Se precisar converter data_admissao para date
        from datetime import datetime
        # data_adm_obj = datetime.strptime(data_admissao, '%Y-%m-%d').date()

        pesquisa = get_object_or_404(
            AvaliacaoPeriodoExperiencia,
            token=token
        )
         
        # Campos do formulário RH
        campos_formulario = [
            'gestor_avaliador','colaborador','data_admissao','cargo','apresenta_iniciativa', 'organizado_atividades', 'adapta_novas_situacoes_clientes',
            'interage_bem_colegas', 'aptidao_lideranca', 'talento_para_funcao', 'pronto_para_colaborar',
            'resultados_esperados', 'colabora_membros_empresa', 'comportamento_etico', 'desiste_facil',
            'reduz_despesas_desperdicios', 'comunica_claro_coerente', 'administra_tempo', 'autoconfiante',
            'empenho_resultados_grupo', 'aceita_opinioes_divergentes', 'relutante_decisoes_grupo',
            'expor_necessidades_perguntas', 'assiduo', 'aceita_ordens_gestor', 'sugestao_critica_elogio', 'aprova_demite', 'token'
        ]
        
        data = {campo: getattr(pesquisa, campo) for campo in campos_formulario}
        return JsonResponse(data)

    except AvaliacaoPeriodoExperiencia.DoesNotExist:
...
def pesquisa(request):
    """View para listar e filtrar pesquisas de satisfação"""

    # Captura os filtros enviados pelo GET
    gestor = request.GET.get("gestor")
    colaborador = request.GET.get("colaborador")
    avaliacao_tipo = request.GET.get("primeira_segunda_avaliacao")
    data_admissao = request.GET.get("data_admissao")

    # Começa com todos os objetos, ordenados
    avaliacoes = AvaliacaoPeriodoExperiencia.objects.all().order_by("-data_avaliacao")
    
    # Aplica filtros se existirem
    if gestor:
        avaliacoes = avaliacoes.filter(gestor_avaliador__icontains=gestor)
    if colaborador:
        avaliacoes = avaliacoes.filter(colaborador__icontains=colaborador)
    if avaliacao_tipo:
        avaliacoes = avaliacoes.filter(primeira_segunda_avaliacao=avaliacao_tipo)
    if data_admissao:
        avaliacoes = avaliacoes.filter(data_admissao=data_admissao)

    # --- INÍCIO DA LÓGICA DE PAGINAÇÃO ---
    
    # 1. Cria o objeto Paginator com o queryset filtrado e define 25 itens por página
    paginator = Paginator(avaliacoes, 10) 

    # 2. Pega o número da página da URL (ex: ?page=2)
    page_number = request.GET.get('page')

    # 3. Obtém o objeto da página para o número solicitado
