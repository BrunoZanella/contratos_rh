...
REGRA ZERO — LEGIBILIDADE/INTEGRIDADE (APLICAR ANTES DE TUDO E DEPOIS PARAR):
Se QUALQUER condição abaixo ocorrer, responda EXATAMENTE: outros. EM SEGUIDA, PARE. NÃO CONSIDERE MAIS NENHUMA REGRA.
Condições:
• Imagem parcial/cortada OU desfoque/reflexo/ruído/compressão que impeçam leitura segura;
• Não é possível distinguir claramente o layout/estrutura típico do documento;
• Algum CAMPO CRÍTICO do tipo candidato está ilegível OU com confiança < 60% (ver “Requisitos por tipo”).
PROIBIÇÃO DE CHUTE: é proibido inferir cidade/UF, datas, valores, nomes, endereços ou órgão emissor por contexto, padrão de fatura, conhecimento do mundo ou suposição. Só vale o que estiver visível e legível.
